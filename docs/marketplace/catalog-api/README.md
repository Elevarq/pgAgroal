...
> **v1.4.2 (harden-then-resubmit).** The public version is now **Elevarq
> packaging v1.4.2** (bundling upstream pgagroal 2.1.0), superseding the
> in-review 1.3.0. `02-add-helm-delivery.json` here is set to 1.4.2; re-host
> the **1.4.2** image + chart into the Marketplace ECR (same steps as the
> 1.3.0 re-host below, with `:1.4.2` tags) before adding the delivery option.
