...
> clears.
>
> **Current public version: v1.4.5 (frontend TLS).** The listing target is now
> **Elevarq packaging v1.4.5** (bundling upstream pgagroal 2.1.0), which ships
> hardened shipped defaults plus optional frontend TLS (client -> pooler). It
> **supersedes** v1.4.3 (and the never-published 1.4.4); re-host the 1.4.5
> image (platform manifests only) + chart and add the 1.4.5 delivery option,
> then retire 1.4.3 with `RestrictDeliveryOptions`. The canonical paste-ready portal copy
...
| Short description | Production-ready pgagroal container for PostgreSQL with non-root security, signed multi-arch images, Kubernetes Helm chart, and zero telemetry. |
| Long description | Elevarq pgAgroal is an open-source (BSD-3-Clause) production-ready container image for pgagroal, the high-performance connection pooler for PostgreSQL. Built from pinned upstream pgagroal 2.1.0 sources on Debian, it is designed for secure, repeatable production deployments. The runtime runs as a non-root user (UID 1000), drops all Linux capabilities, uses a read-only root filesystem, and defaults to the RuntimeDefault seccomp profile. Multi-architecture images (linux/amd64 and linux/arm64) are cosign-signed using GitHub OIDC and include an SBOM and SLSA provenance. A production-ready Helm chart simplifies Kubernetes deployment with liveness and readiness probes, a PodDisruptionBudget, least-privilege security contexts, and optional Prometheus metrics. Elevarq pgAgroal contains no telemetry and sends no data outside your environment. |
| Categories | Infrastructure Software (validated; AWS has no "Database" category) |
| Logo | `https://elevarq-marketplace-public.s3.amazonaws.com/logos/elevarq-512.png` (512×512 transparent PNG, Elevarq mark from website `app/icon.svg`) |
...
the v1.4.5 default does not pass unknown users through to the backend:
...
6. [x] Host the product logo in S3 (`LogoUrl` requires an S3 URL — a non-S3
       https host fails `INVALID_MEDIA`). Scoped public bucket
       `elevarq-marketplace-public`, `logos/` prefix.
7. [x] **`UpdateInformation`** (`catalog-api/03-update-information.json`) —
       product title, descriptions, highlights, category, keywords, logo.
       Verified `SUCCEEDED`; product still Draft.
8. [ ] **`ReleaseProduct`** — Draft → Limited (unblocks AddDeliveryOptions).
9. [ ] **`AddDeliveryOptions`** (`catalog-api/02-add-helm-delivery.json`) — add
       the Helm version (now allowed, product is Limited).
10. [ ] **Free `Offer@1.0`** (`CreateOffer` + `ReleaseOffer`) — legal/EULA/
        support terms. **Gated on EULA legal sign-off (§4).**
11. [ ] **`UpdateVisibility: Public`** — public launch (AWS manual review).
        Gated on the EULA + explicit go.

> The whole sequence is Catalog-API-driven — no AMMP web UI required. See
> [`catalog-api/README.md`](catalog-api/README.md) for the verified steps.
